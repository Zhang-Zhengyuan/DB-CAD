...
主入口、密码管理、Docker Neo4j 部署的**完整说明**见：

- [`../DEPLOYMENT.md`](../DEPLOYMENT.md)

下面只保留**实验性 PostgreSQL 直存 SAT demo**——这部分与主 Neo4j 部署完全独立，作为可选 demo。

---

## PostgreSQL 直存 SAT 文本 mini-demo（实验性）

这是一个**完全独立**于主 Neo4j 一键部署的可选小 demo，用于演示"直接用关系数据库存 SAT 文本、绕开 Neo4j"的可行性。**它不会启动主流程、不影响现有 Neo4j 一键部署**。
...
> **注**：最初规划用 `libpqxx`，但 vcpkg 给出的 libpqxx 8.0.1（动态 + Meson 编译）在本机链接时会产生 `STATUS_STACK_BUFFER_OVERRUN` 之类 CRT/堆栈保护相关的崩溃；所以最终 demo 直接调 `libpq`（`libpq-fe.h`）。**功能层面等价，依赖更小**——`libpq.dll` + `libssl-3-x64.dll` + `libcrypto-3-x64.dll` + `z.dll` + `lz4.dll` 共 5 个 DLL 即可。
...
唯一被修改的主流程文件是 `mainwindow.cpp` / `mainwindow.h` —— 加了一个独立的顶级菜单 **"PostgreSQL(P)"** 与 4 个 slot，slot 实现**完全不碰** `saveFile()`、`loadFile()`、`storage_bridge_service`、`backend_api_client`、FastAPI 路由。
...
**用浏览器看表数据**：http://localhost:5050 → 登录 → Add New Server：`Host=dbcad-postgres-demo, Port=5432, Maintenance db=postgres, User=postgres, Password=admin` → 在左侧找到 `dbcad_demo / Schemas / public / Tables / dbcad_pg_demo` → 右键 View/Edit Data。
